...
module.exports = function(connect){
  ...
  function RedisStore(options) {
    ...
    this._encryptSession = function (sess) {
      var cipher = crypto.createCipher('aes-256-cbc', options.secret);
      var content = JSON.stringify(sess);
      var encrypted = cipher.update(content, 'utf-8', 'hex');
      encrypted += cipher.final('hex');
      return {encrypted: true, content: encrypted};
    };

    this._decryptSession = function (sess) {
      var decipher  = crypto.createDecipher('aes-256-cbc', options.secret);
      if (!sess || !sess.encrypted) {
        return sess;
      }
      var content = sess.content;
      var decrypted = decipher.update(content, 'hex', 'utf-8');
